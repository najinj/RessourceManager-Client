import {
    ...
    REMOVE_ASSET_FROM_SPACE_REQUEST} from "./types";
  ...
  export function fetchSpaces() {
    return dispatch => {
      dispatch({ type: FETCH_SPACE_REQUEST });
      SpaceServices.fetchSpaces().then(
        ...
        err => {
          console.log(FETCH_SPACE_FAILURE, err);
          dispatch({ type: FETCH_SPACE_FAILURE });
        }
      );
    };
  }
  ...
  export function AddAssetToSpace(assetId,spaceId){
    return dispatch =>{
        ...
        SpaceServices.AddAssetToSpace(assetId,spaceId).then(response=>{
  ...
  export function RemoveAssetFromSpace(assetId,spaceId){
    return dispatch =>{
        ...
        SpaceServices.RemoveAssetFromSpace(assetId,spaceId).then(response=>{
            dispatch({type:REMOVE_ASSET_FROM_SPACE_SUCCESS,payload:response.data})
        },err=>{
